...
def p_train(make_obs_ph_n, act_space_n, p_index, p_func, q_func, optimizer, grad_norm_clipping=None, scope="trainer", reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        p = p_func(obs_ph_n[p_index], int(act_pdtype_n[p_index].param_shape()[0]), scope="p_func")
        ...
        kl_loss = tf.reduce_mean(tf.square(act_pd.flatparam()))
        ...
        # target network
        target_p = p_func(obs_ph_n[p_index], int(act_pdtype_n[p_index].param_shape()[0]), scope="target_p_func")
